...
coloured* colourFromString( const std::string& s, const std::string& fields, const QColor4ub& backgroundcolour )
{
    ...
    comma::graphics::View::coloured* c;
    try
    {
        if( hasId )
        {
            if( hasScalar )
            {
                ...
                c = new comma::graphics::View::ById( backgroundcolour, from, to );
            }
            else
            {
                c = new comma::graphics::View::ById( backgroundcolour );
            }
        }
        else if( ( r && g && b ) || a )
        {
            c = new comma::graphics::View::ByRGB;
        }
        else if( hasScalar )
        {
            ...
            c = new comma::graphics::View::ByScalar( from, to, from_color, to_color );
        }
        else
        {
            c = new comma::graphics::View::Fixed( s );
        }
    }
    catch( ... )
    {
        ...
        c = new comma::graphics::View::ByHeight( *from, to, from_color, to_color, cyclic, linear,sharp );
    }
    return c;
}
