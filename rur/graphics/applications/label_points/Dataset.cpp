...
void BasicDataset::init()
{
    ...
    m_vertices.reset( new Qt3D::VertexBuffer( m_points.size() ) );
...
void Dataset::save()
{
    ...
    boost::scoped_ptr< Csv::AsciiOutputStream< PointWithId > > ascii;
    boost::scoped_ptr< Csv::BinaryOutputStream< PointWithId > > binary;
    if( m_options.binary() ) { binary.reset( new Csv::BinaryOutputStream< PointWithId >( ofs, m_options.format().string(), fields, false ) ); }
    else { ascii.reset( new Csv::AsciiOutputStream< PointWithId >( ofs, fields, m_options.delimiter, false ) ); }
    std::size_t count = 0;
